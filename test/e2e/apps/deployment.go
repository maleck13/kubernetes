...
func testDeleteDeployment(f *framework.Framework) {
	...
	deploy, err := c.AppsV1().Deployments(ns).Create(ctx, d)
	Expect(err).NotTo(HaveOccurred())
...
func testRollingUpdateDeployment(f *framework.Framework) {
	...
	_, err := c.AppsV1().ReplicaSets(ns).Create(ctx, rs)
	Expect(err).NotTo(HaveOccurred())
...
func testDeploymentCleanUpPolicy(f *framework.Framework) {
	...
	_, err := c.AppsV1().ReplicaSets(ns).Create(ctx, newRS(rsName, replicas, rsPodLabels, NginxImageName, NginxImage))
	Expect(err).NotTo(HaveOccurred())
	...
	_, err = c.AppsV1().Deployments(ns).Create(ctx,d)
	Expect(err).NotTo(HaveOccurred())
...
func testRolloverDeployment(f *framework.Framework) {
	...
	rsReplicas := int32(1)
	_, err := c.AppsV1().ReplicaSets(ns).Create(ctx, newRS(rsName, rsReplicas, rsPodLabels, NginxImageName, NginxImage))
	Expect(err).NotTo(HaveOccurred())
	// Verify that the required pods have come up.
	...
	_, err = c.AppsV1().Deployments(ns).Create(ctx, newDeployment)
	Expect(err).NotTo(HaveOccurred())
...
func testIterativeDeployments(f *framework.Framework) {
	...
	framework.Logf("Creating deployment %q", deploymentName)
	deployment, err := c.AppsV1().Deployments(ns).Create(ctx, d)
	Expect(err).NotTo(HaveOccurred())
