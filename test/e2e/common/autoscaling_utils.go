...
func (rc *ResourceConsumer) GetReplicas() int {
	...
	case KindDeployment:
		deployment, err := rc.clientSet.ExtensionsV1beta1().Deployments(rc.nsName).Get(ctx, rc.name, metav1.GetOptions{})
		framework.ExpectNoError(err)
		if deployment == nil {
	...
	case KindReplicaSet:
		rs, err := rc.clientSet.ExtensionsV1beta1().ReplicaSets(rc.nsName).Get(ctx, rc.name, metav1.GetOptions{})
		framework.ExpectNoError(err)
		if rs == nil {
...
func (rc *ResourceConsumer) CleanUp() {
	...
	framework.ExpectNoError(rc.clientSet.CoreV1().Services(rc.nsName).Delete(ctx, rc.name, nil))
	...
	framework.ExpectNoError(rc.clientSet.CoreV1().Services(rc.nsName).Delete(ctx, rc.controllerName, nil))
}
...
func runServiceAndWorkloadForResourceConsumer(c clientset.Interface, internalClient internalclientset.Interface, ns, name string, kind schema.GroupVersionKind, replicas int, cpuLimitMillis, memLimitMb int64, podAnnotations, serviceAnnotations map[string]string) {
	...
	ctx := context.TODO()
	_, err := c.CoreV1().Services(ns).Create(ctx, &v1.Service{
