...
var _ = SIGDescribe("ResourceQuota", func() {
	...
	It("should create a ResourceQuota and capture the life of a service.", func() {
		...
		service, err = f.ClientSet.CoreV1().Services(f.Namespace.Name).Create(ctx, service)
		Expect(err).NotTo(HaveOccurred())
		...
		err = f.ClientSet.CoreV1().Services(f.Namespace.Name).Delete(ctx, service.Name, nil)
		Expect(err).NotTo(HaveOccurred())
	...
	It("should create a ResourceQuota and capture the life of a secret.", func() {
		...
		err = f.ClientSet.CoreV1().Secrets(f.Namespace.Name).Delete(ctx, secret.Name, nil)
		Expect(err).NotTo(HaveOccurred())
	...
	It("[Feature:Initializers] should create a ResourceQuota and capture the life of an uninitialized pod.", func() {
		...
		createdPod, err := f.ClientSet.CoreV1().Pods(f.Namespace.Name).Get(ctx, podName, metav1.GetOptions{})
		Expect(err).NotTo(HaveOccurred())
		...
		By("Deleting the pod")
		err = f.ClientSet.CoreV1().Pods(f.Namespace.Name).Delete(ctx, createdPod.Name, metav1.NewDeleteOptions(0))
		Expect(err).NotTo(HaveOccurred())
		...
		pod = newTestPodForQuota(f, podName, requests, v1.ResourceList{})
		pod.Initializers = &metav1.Initializers{Pending: []metav1.Initializer{{Name: "unhandled"}}}
		_, err = f.ClientSet.CoreV1().Pods(f.Namespace.Name).Create(ctx, pod)
		// because no one is handling the initializer, server will return a 504 timeout
		if err != nil && !errors.IsTimeout(err) {
			framework.Failf("expect err to be timeout error, got %v", err)
		}
	...
	It("should create a ResourceQuota and capture the life of a pod.", func() {
		...
		_, err = f.ClientSet.CoreV1().Pods(f.Namespace.Name).Update(ctx, podToUpdate)
		Expect(err).To(HaveOccurred())
	...
	It("should create a ResourceQuota and capture the life of a configMap.", func() {
		...
		err = f.ClientSet.CoreV1().ConfigMaps(f.Namespace.Name).Delete(ctx, configMap.Name, nil)
		Expect(err).NotTo(HaveOccurred())
	...
	It("should create a ResourceQuota and capture the life of a replication controller.", func() {
		...
		replicationController, err = f.ClientSet.CoreV1().ReplicationControllers(f.Namespace.Name).Create(ctx, replicationController)
		Expect(err).NotTo(HaveOccurred())
		...
		err = f.ClientSet.CoreV1().ReplicationControllers(f.Namespace.Name).Delete(ctx, replicationController.Name, nil)
		Expect(err).NotTo(HaveOccurred())
	...
	It("should create a ResourceQuota and capture the life of a replica set.", func() {
		...
		replicaSet, err = f.ClientSet.ExtensionsV1beta1().ReplicaSets(f.Namespace.Name).Create(ctx, replicaSet)
		Expect(err).NotTo(HaveOccurred())
		...
		err = f.ClientSet.ExtensionsV1beta1().ReplicaSets(f.Namespace.Name).Delete(ctx, replicaSet.Name, nil)
		Expect(err).NotTo(HaveOccurred())
	...
	It("should create a ResourceQuota and capture the life of a persistent volume claim. [sig-storage]", func() {
		...
		pvc, err = f.ClientSet.CoreV1().PersistentVolumeClaims(f.Namespace.Name).Create(ctx, pvc)
		Expect(err).NotTo(HaveOccurred())
		...
		err = f.ClientSet.CoreV1().PersistentVolumeClaims(f.Namespace.Name).Delete(ctx, pvc.Name, nil)
		Expect(err).NotTo(HaveOccurred())
