...
func runDensityTest(dtc DensityTestConfig, testPhaseDurations *timer.TestPhaseTimer) time.Duration {
	...
	podList, err := dtc.ClientSets[0].CoreV1().Pods(metav1.NamespaceAll).List(context.TODO(), metav1.ListOptions{})
	framework.ExpectNoError(err)
		...
		It(name, func() {
			...
			if itArg.runLatencyTest {
					...
					latencyPodsStore, controller := cache.NewInformer(
						&cache.ListWatch{
							ListFunc: func(options metav1.ListOptions) (runtime.Object, error) {
								...
								obj, err := c.CoreV1().Pods(nsName).List(ctx, options)
								return runtime.Object(obj), err
							},
							WatchFunc: func(options metav1.ListOptions) (watch.Interface, error) {
								options.LabelSelector = labels.SelectorFromSet(labels.Set{"type": additionalPodsPrefix}).String()
								return c.CoreV1().Pods(nsName).Watch(ctx, options)
							},
						},
						&v1.Pod{},
						0,
				...
				for i := 0; i < len(namespaces); i++ {
					...
					schedEvents, err := c.CoreV1().Events(nsName).List(ctx, options)
					framework.ExpectNoError(err)
