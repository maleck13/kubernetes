...
var _ = SIGDescribe("ServiceAccounts", func() {
	...
	It("should ensure a single API token exists", func() {
		...
		framework.ExpectNoError(f.ClientSet.CoreV1().Secrets(f.Namespace.Name).Delete(ctx, secrets[0].Name, nil))
		...
		By("deleting the reference to the service account token")
		{
			...
			framework.ExpectNoError(err)
			sa.Secrets = nil
			_, updateErr := f.ClientSet.CoreV1().ServiceAccounts(f.Namespace.Name).Update(ctx, sa)
	...
	framework.ConformanceIt("should mount an API token into pods ", func() {
		...
		framework.ExpectNoError(wait.Poll(time.Millisecond*500, framework.ServiceAccountProvisionTimeout, func() (bool, error) {
			...
			for _, secretRef := range sa.Secrets {
				secret, err := f.ClientSet.CoreV1().Secrets(f.Namespace.Name).Get(ctx, secretRef.Name, metav1.GetOptions{})
				if err != nil {
	...
	framework.ConformanceIt("should allow opting out of API token automount ", func() {
		...
		mountSA, err = f.ClientSet.CoreV1().ServiceAccounts(f.Namespace.Name).Create(ctx, mountSA)
		framework.ExpectNoError(err)
		nomountSA, err = f.ClientSet.CoreV1().ServiceAccounts(f.Namespace.Name).Create(ctx, nomountSA)
		framework.ExpectNoError(err)

		// Standard get, update retry loop
		framework.ExpectNoError(wait.Poll(time.Millisecond*500, framework.ServiceAccountProvisionTimeout, func() (bool, error) {
			By("getting the auto-created API token")
			sa, err := f.ClientSet.CoreV1().ServiceAccounts(f.Namespace.Name).Get(ctx, mountSA.Name, metav1.GetOptions{})
			if apierrors.IsNotFound(err) {
		...
		for _, tc := range testcases {
			...
			createdPod, err := f.ClientSet.CoreV1().Pods(f.Namespace.Name).Create(ctx, pod)
			framework.ExpectNoError(err)
