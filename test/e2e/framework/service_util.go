...
func (j *ServiceTestJig) CreateTCPServiceWithPort(namespace string, tweak func(svc *v1.Service), port int32) *v1.Service {
	...
	result, err := j.Client.CoreV1().Services(namespace).Create(context.TODO(), svc)
	if err != nil {
...
func (j *ServiceTestJig) CreateServiceWithServicePort(labels map[string]string, namespace string, ports []v1.ServicePort) (*v1.Service, error) {
	...
	return j.Client.CoreV1().Services(namespace).Create(context.TODO(), service)
}
...
func (j *ServiceTestJig) GetEndpointNodes(svc *v1.Service) map[string][]string {
	...
	endpoints, err := j.Client.CoreV1().Endpoints(svc.Namespace).Get(context.TODO(), svc.Name, metav1.GetOptions{})
	if err != nil {
...
func (j *ServiceTestJig) WaitForEndpointOnNode(namespace, serviceName, nodeName string) {
	err := wait.PollImmediate(Poll, LoadBalancerCreateTimeoutDefault, func() (bool, error) {
		endpoints, err := j.Client.CoreV1().Endpoints(namespace).Get(context.TODO(), serviceName, metav1.GetOptions{})
		if err != nil {
...
func (j *ServiceTestJig) UpdateService(namespace, name string, update func(*v1.Service)) (*v1.Service, error) {
	ctx := context.TODO()
	for i := 0; i < 3; i++ {
		service, err := j.Client.CoreV1().Services(namespace).Get(ctx, name, metav1.GetOptions{})
		if err != nil {
			return nil, fmt.Errorf("Failed to get Service %q: %v", name, err)
		}
		update(service)
		service, err = j.Client.CoreV1().Services(namespace).Update(ctx, service)
		if err == nil {
			return service, nil
		}
...
func (j *ServiceTestJig) waitForConditionOrFail(namespace, name string, timeout time.Duration, message string, conditionFn func(*v1.Service) bool) *v1.Service {
	...
	pollFunc := func() (bool, error) {
		svc, err := j.Client.CoreV1().Services(namespace).Get(context.TODO(), name, metav1.GetOptions{})
		if err != nil {
			return false, err
		}
		if conditionFn(svc) {
...
func (j *ServiceTestJig) CreatePDBOrFail(namespace string, rc *v1.ReplicationController) *policyv1beta1.PodDisruptionBudget {
	...
	newPdb, err := j.Client.PolicyV1beta1().PodDisruptionBudgets(namespace).Create(context.TODO(), pdb)
	if err != nil {
...
func (j *ServiceTestJig) RunOrFail(namespace string, tweak func(rc *v1.ReplicationController)) *v1.ReplicationController {
	...
	result, err := j.Client.CoreV1().ReplicationControllers(namespace).Create(context.TODO(), rc)
	if err != nil {
...
func (j *ServiceTestJig) waitForPdbReady(namespace string) error {
	...
	for start := time.Now(); time.Since(start) < timeout; time.Sleep(2 * time.Second) {
		pdb, err := j.Client.PolicyV1beta1().PodDisruptionBudgets(namespace).Get(context.TODO(), j.Name, metav1.GetOptions{})
		if err != nil {
			return err
		}
...
func (j *ServiceTestJig) waitForPodsCreated(namespace string, replicas int) ([]string, error) {
	...
	for start := time.Now(); time.Since(start) < timeout; time.Sleep(2 * time.Second) {
		...
		pods, err := j.Client.CoreV1().Pods(namespace).List(context.TODO(), options)
		if err != nil {
			return nil, err
		}
...
func (t *ServiceTestFixture) Cleanup() []error {
	...
	for rcName := range t.rcs {
		...
		err := retry.RetryOnConflict(retry.DefaultRetry, func() error {
			...
			old, err := t.Client.CoreV1().ReplicationControllers(t.Namespace).Get(ctx, rcName, metav1.GetOptions{})
			if err != nil {
			...
			x := int32(0)
			old.Spec.Replicas = &x
			if _, err := t.Client.CoreV1().ReplicationControllers(t.Namespace).Update(ctx, old); err != nil {
				if errors.IsNotFound(err) {
					return nil
				}
				return err
		...
		if err := t.Client.CoreV1().ReplicationControllers(t.Namespace).Delete(ctx, rcName, nil); err != nil {
			if !errors.IsNotFound(err) {
				errs = append(errs, err)
			}
		}
	...
	for serviceName := range t.services {
		...
		err := t.Client.CoreV1().Services(t.Namespace).Delete(ctx, serviceName, nil)
		if err != nil {
			if !errors.IsNotFound(err) {
				errs = append(errs, err)
			}
...
func UpdateService(c clientset.Interface, namespace, serviceName string, update func(*v1.Service)) (*v1.Service, error) {
	...
	for i := 0; i < 3; i++ {
		...
		service, err = c.CoreV1().Services(namespace).Update(ctx, service)
...
func ValidateEndpointsOrFail(c clientset.Interface, namespace, serviceName string, expectedEndpoints PortsByPodName) {
	...
	for start := time.Now(); time.Since(start) < ServiceStartTimeout; time.Sleep(1 * time.Second) {
		endpoints, err := c.CoreV1().Endpoints(namespace).Get(ctx, serviceName, metav1.GetOptions{})
		if err != nil {
...
func StartServeHostnameService(c clientset.Interface, internalClient internalclientset.Interface, svc *v1.Service, ns string, replicas int) ([]string, string, error) {
	...
	_, err := c.CoreV1().Services(ns).Create(ctx, svc)
	if err != nil {
		return podNames, "", err
	}
