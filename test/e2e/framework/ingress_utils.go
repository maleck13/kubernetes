...
func createTLSSecret(kubeClient clientset.Interface, namespace, secretName string, hosts ...string) (host string, rootCA, privKey []byte, err error) {
	...
	if s, err = kubeClient.CoreV1().Secrets(namespace).Get(ctx, secretName, metav1.GetOptions{}); err == nil {
		...
		s.Data = secret.Data
		_, err = kubeClient.CoreV1().Secrets(namespace).Update(ctx, s)
	} else {
		Logf("Creating secret %v in ns %v with hosts %v", secret.Name, namespace, host)
		_, err = kubeClient.CoreV1().Secrets(namespace).Create(ctx, secret)
	}
...
func (j *IngressTestJig) CreateIngress(manifestPath, ns string, ingAnnotations map[string]string, svcAnnotations map[string]string) {
	...
	if len(svcAnnotations) > 0 {
		...
		for _, svc := range svcList.Items {
			...
			_, err = j.Client.CoreV1().Services(ns).Update(ctx, &svc)
			ExpectNoError(err)
		}
	}
...
func (j *IngressTestJig) SetUpBacksideHTTPSIngress(cs clientset.Interface, namespace string, staticIPName string) (*extensions.Deployment, *v1.Service, *extensions.Ingress, error) {
	ctx := context.TODO()
	deployCreated, err := cs.ExtensionsV1beta1().Deployments(namespace).Create(ctx, generateBacksideHTTPSDeploymentSpec())
	if err != nil {
		return nil, nil, nil, err
	}
	svcCreated, err := cs.CoreV1().Services(namespace).Create(ctx, generateBacksideHTTPSServiceSpec())
	if err != nil {
		return nil, nil, nil, err
	}
