...
var _ = utils.SIGDescribe("vsphere cloud provider stress [Feature:vsphere]", func() {
	...
	It("vsphere stress tests", func() {
		...
		for index, scname := range scNames {
			...
			switch scname {
			case storageclass1:
				sc, err = client.StorageV1().StorageClasses().Create(ctx, getVSphereStorageClassSpec(storageclass1, nil))
			case storageclass2:
				...
				sc, err = client.StorageV1().StorageClasses().Create(ctx, getVSphereStorageClassSpec(storageclass2, scVSanParameters))
			case storageclass3:
				...
				sc, err = client.StorageV1().StorageClasses().Create(ctx, getVSphereStorageClassSpec(storageclass3, scSPBMPolicyParameters))
			case storageclass4:
				...
				sc, err = client.StorageV1().StorageClasses().Create(ctx, scWithDatastoreSpec)
			}
			Expect(sc).NotTo(BeNil())
			Expect(err).NotTo(HaveOccurred())
			defer client.StorageV1().StorageClasses().Delete(ctx, scname, nil)
			scArrays[index] = sc
		}
