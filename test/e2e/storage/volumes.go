...
var _ = utils.SIGDescribe("Volumes", func() {
	...
	Describe("Ceph RBD [Feature:Volumes]", func() {
		It("should be mountable", func() {
			...
			defer func() {
				secClient.Delete(ctx, config.Prefix+"-secret", nil)
			}()

			if _, err := secClient.Create(ctx, &secret); err != nil {
	...
	Describe("CephFS [Feature:Volumes]", func() {
		It("should be mountable", func() {
			...
			defer func() {
				if err := cs.CoreV1().Secrets(namespace.Name).Delete(ctx, secret.Name, nil); err != nil {
					framework.Failf("unable to delete secret %v: %v", secret.Name, err)
				}
			}()

			var err error
			if secret, err = cs.CoreV1().Secrets(namespace.Name).Create(ctx, secret); err != nil {
