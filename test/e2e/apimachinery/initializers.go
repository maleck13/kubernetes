...
var _ = SIGDescribe("Initializers [Feature:Initializers]", func() {
	...
	It("should be invisible to controllers by default", func() {
		...
		ch := make(chan struct{})
		go func() {
			_, err := c.CoreV1().Pods(ns).Create(ctx, newUninitializedPod(podName))
		...
		err := wait.PollImmediate(2*time.Second, 15*time.Second, func() (bool, error) {
			p, err := c.CoreV1().Pods(ns).Get(ctx, podName, metav1.GetOptions{})
			if err != nil {
		...
		pod, err = c.CoreV1().Pods(ns).Update(ctx, pod)
		Expect(err).NotTo(HaveOccurred())

		// verify the list call filters out uninitialized pods
		pods, err := c.CoreV1().Pods(ns).List(ctx, metav1.ListOptions{IncludeUninitialized: true})
		Expect(err).NotTo(HaveOccurred())
		...
		_, err = c.CoreV1().Pods(ns).Update(ctx, pod)
	...
	It("should dynamically register and apply initializers to pods [Serial]", func() {
		...
		_, err := c.AdmissionregistrationV1alpha1().InitializerConfigurations().Create(ctx, &v1alpha1.InitializerConfiguration{
		...
		go func() {
			defer close(ch)
			_, err := c.CoreV1().Pods(ns).Create(ctx, newInitPod(podName))
			Expect(err).NotTo(HaveOccurred())
		}()
		...
		var pod *v1.Pod
		err = wait.PollImmediate(2*time.Second, 15*time.Second, func() (bool, error) {
			pod, err = c.CoreV1().Pods(ns).Get(ctx, podName, metav1.GetOptions{IncludeUninitialized: true})
			if errors.IsNotFound(err) {
				return false, nil
			}
			if err != nil {
	...
	It("don't cause replicaset controller creating extra pods if the initializer is not handled [Serial]", func() {
		...
		// create a replicaset
		persistedRS, err := c.ExtensionsV1beta1().ReplicaSets(ns).Create(ctx, newReplicaset())
		Expect(err).NotTo(HaveOccurred())
		...
		persistedRS, err = c.ExtensionsV1beta1().ReplicaSets(ns).Patch(ctx, persistedRS.Name, types.StrategicMergePatchType, patch)
		Expect(err).NotTo(HaveOccurred())
		...
		pods, err := c.CoreV1().Pods(ns).List(ctx, listOptions)
		Expect(err).NotTo(HaveOccurred())
	...
	It("will be set to nil if a patch removes the last pending initializer", func() {
		...
		uninitializedPod, err := c.CoreV1().Pods(ns).Get(ctx, podName, metav1.GetOptions{})
		Expect(err).NotTo(HaveOccurred())
		...
		patchedPod, err := c.CoreV1().Pods(ns).Patch(ctx, uninitializedPod.Name, types.StrategicMergePatchType, []byte(patch))
		Expect(err).NotTo(HaveOccurred())
...
func removeInitializersFromAllPods(c clientset.Interface, initializerName string) {
	ctx := context.TODO()
	pods, err := c.CoreV1().Pods("").List(ctx, metav1.ListOptions{IncludeUninitialized: true})
	if err != nil {
		return
	}
	for _, p := range pods.Items {
		...
		err := clientretry.RetryOnConflict(clientretry.DefaultRetry, func() error {
			pod, err := c.CoreV1().Pods(p.Namespace).Get(ctx, p.Name, metav1.GetOptions{IncludeUninitialized: true})
			if err != nil {
				if errors.IsNotFound(err) {
					return nil
				}
			...
			_, err = c.CoreV1().Pods(p.Namespace).Update(ctx, pod)
			return err
		})
		if err != nil {
