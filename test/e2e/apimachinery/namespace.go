...
func ensurePodsAreRemovedWhenNamespaceIsDeleted(f *framework.Framework) {
	...
	pod, err = f.ClientSet.CoreV1().Pods(namespace.Name).Create(ctx, pod)
	Expect(err).NotTo(HaveOccurred())
	...
	go func() {
		_, err = f.ClientSet.CoreV1().Pods(namespace.Name).Create(ctx, podB)
	...
	err = f.ClientSet.CoreV1().Namespaces().Delete(ctx, namespace.Name, nil)
	Expect(err).NotTo(HaveOccurred())
	...
	_, err = f.ClientSet.CoreV1().Pods(namespace.Name).Get(ctx, podB.Name, metav1.GetOptions{IncludeUninitialized: true})
	Expect(err).To(HaveOccurred())
}
...
func ensureServicesAreRemovedWhenNamespaceIsDeleted(f *framework.Framework) {
	...
	service, err = f.ClientSet.CoreV1().Services(namespace.Name).Create(ctx, service)
	Expect(err).NotTo(HaveOccurred())
