...
func createPriorityClasses(f *framework.Framework) func() {
	...
	for className, priority := range priorityClasses {
		_, err := f.ClientSet.SchedulingV1alpha1().PriorityClasses().Create(ctx, &v1alpha1.PriorityClass{ObjectMeta: metav1.ObjectMeta{Name: className}, Value: priority})
		Expect(err == nil || errors.IsAlreadyExists(err)).To(Equal(true))
	}

	return func() {
		for className := range priorityClasses {
			f.ClientSet.SchedulingV1alpha1().PriorityClasses().Delete(ctx, className, nil)
		}
	}
}
