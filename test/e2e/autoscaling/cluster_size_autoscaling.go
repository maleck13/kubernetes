...
func runDrainTest(f *framework.Framework, migSizes map[string]int, namespace string, podsPerNode, pdbSize int, verifyFunction func(int)) {
	...
	_, err = f.ClientSet.PolicyV1beta1().PodDisruptionBudgets(namespace).Create(ctx, pdb)

	defer func() {
		f.ClientSet.PolicyV1beta1().PodDisruptionBudgets(namespace).Delete(ctx, pdb.Name, &metav1.DeleteOptions{})
	}()
...
func makeNodeUnschedulable(c clientset.Interface, node *v1.Node) error {
	...
	for j := 0; j < 3; j++ {
		...
		_, err = c.CoreV1().Nodes().Update(ctx, freshNode)
		if err == nil {
			return nil
		}
	...
	for i, node := range nodes {
		...
		for j := 0; j < 3; j++ {
			*rc.Spec.Replicas = int32((i + 1) * podsPerNode)
			rc, err = f.ClientSet.CoreV1().ReplicationControllers(namespace).Update(ctx, rc)
			if err == nil {
				break
			}
			if !errors.IsConflict(err) {
...
func addKubeSystemPdbs(f *framework.Framework) (func(), error) {
	...
	cleanup := func() {
		...
		for _, newPdbName := range newPdbs {
			...
			err := f.ClientSet.PolicyV1beta1().PodDisruptionBudgets("kube-system").Delete(ctx, newPdbName, &metav1.DeleteOptions{})
			if err != nil {
	...
	for _, pdbData := range pdbsToAdd {
		...
		_, err := f.ClientSet.PolicyV1beta1().PodDisruptionBudgets("kube-system").Create(ctx, pdb)
...
func createPriorityClasses(f *framework.Framework) func() {
	...
	for className, priority := range priorityClasses {
		_, err := f.ClientSet.SchedulingV1beta1().PriorityClasses().Create(ctx, &schedulerapi.PriorityClass{ObjectMeta: metav1.ObjectMeta{Name: className}, Value: priority})
		Expect(err == nil || errors.IsAlreadyExists(err)).To(Equal(true))
	}

	return func() {
		for className := range priorityClasses {
			f.ClientSet.SchedulingV1beta1().PriorityClasses().Delete(ctx, className, nil)
		}
	}
}
