...
var _ = SIGDescribe("NoSNAT [Feature:NoSNAT] [Slow]", func() {
	...
	It("Should be able to send traffic between Pods without SNAT", func() {
		...
		_, err = pc.Create(ctx, newTestProxyPod(node.Name))
		framework.ExpectNoError(err)

		By("waiting for all of the no-snat-test pods to be scheduled and running")
		err = wait.PollImmediate(10*time.Second, 1*time.Minute, func() (bool, error) {
			pods, err := pc.List(ctx, metav1.ListOptions{LabelSelector: "no-snat-test"})
			if err != nil {
				return false, err
			}
		...
		err = wait.PollImmediate(10*time.Second, 1*time.Minute, func() (bool, error) {
			pod, err := pc.Get(ctx, "no-snat-test-proxy", metav1.GetOptions{})
			if err != nil {
				return false, err
			}
			if pod.Status.Phase != v1.PodRunning {
