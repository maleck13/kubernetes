	...
	framework.KubeDescribe("SystemLogMonitor", func() {
		...
		BeforeEach(func() {
			...
			*hostPathType = v1.HostPathType(string(v1.HostPathFileOrCreate))
			...
			pod, err := f.PodClient().Get(ctx, name, metav1.GetOptions{})
			Expect(err).NotTo(HaveOccurred())
		...
		AfterEach(func() {
			...
			f.PodClient().Delete(ctx, name, metav1.NewDeleteOptions(0))
			...
			c.CoreV1().ConfigMaps(ns).Delete(ctx, configName, nil)
			By("Clean up the events")
			Expect(c.CoreV1().Events(eventNamespace).DeleteCollection(ctx, metav1.NewDeleteOptions(0), eventListOptions)).To(Succeed())
