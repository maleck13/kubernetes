...
func main() {
	...
	for start := time.Now(); time.Since(start) < nodeListTimeout; time.Sleep(2 * time.Second) {
		nodes, err = client.Core().Nodes().List(ctx, metav1.ListOptions{})
		if err == nil {
			break
		}
	...
	got, err := client.Core().Namespaces().Create(ctx, &api.Namespace{ObjectMeta: metav1.ObjectMeta{GenerateName: "serve-hostnames-"}})
	if err != nil {
		glog.Fatalf("Failed to create namespace: %v", err)
	}
	ns := got.Name
	defer func(ns string) {
		if err := client.Core().Namespaces().Delete(ctx, ns, nil); err != nil {
			...
			for i := 0; i < int(namespaceDeleteTimeout/time.Second); i++ {
				if _, err := client.Core().Namespaces().Get(ctx, ns, metav1.GetOptions{}); err != nil {
	...
	for start := time.Now(); time.Since(start) < serviceCreateTimeout; time.Sleep(2 * time.Second) {
		t := time.Now()
		svc, err = client.Core().Services(ns).Create(ctx, &api.Service{
	...
	defer func() {
		...
		for start := time.Now(); time.Since(start) < deleteTimeout; time.Sleep(1 * time.Second) {
			if err := client.Core().Services(ns).Delete(ctx, svc.Name, nil); err == nil {
				return
			}
	...
	defer func() {
		...
		for _, podName := range podNames {
			for start := time.Now(); time.Since(start) < deleteTimeout; time.Sleep(1 * time.Second) {
				if err = client.Core().Pods(ns).Delete(ctx, podName, nil); err == nil {
					break
				}
	...
	for _, podName := range podNames {
		...
		for start := time.Now(); time.Since(start) < podStartTimeout; time.Sleep(5 * time.Second) {
			pod, err = client.Core().Pods(ns).Get(ctx, podName, metav1.GetOptions{})
			if err != nil {
