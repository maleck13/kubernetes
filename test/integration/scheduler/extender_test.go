...
func DoTestPodScheduling(ns *v1.Namespace, t *testing.T, cs clientset.Interface) {
	...
	defer cs.CoreV1().Nodes().DeleteCollection(ctx, nil, metav1.ListOptions{})
	...
	for ii := 0; ii < 5; ii++ {
		...
		if _, err := cs.CoreV1().Nodes().Create(ctx, node); err != nil {
	...
	myPod, err := cs.CoreV1().Pods(ns.Name).Create(ctx, pod)
	if err != nil {
