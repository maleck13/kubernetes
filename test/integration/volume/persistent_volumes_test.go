...
func TestPersistentVolumeRecycler(t *testing.T) {
	...
	defer testClient.Core().PersistentVolumes().DeleteCollection(ctx, nil, metav1.ListOptions{})
	...
	_, err := testClient.CoreV1().PersistentVolumes().Create(ctx, pv)
	if err != nil {
		t.Errorf("Failed to create PersistentVolume: %v", err)
	}
	glog.V(2).Infof("TestPersistentVolumeRecycler pvc created")

	_, err = testClient.CoreV1().PersistentVolumeClaims(ns.Name).Create(ctx, pvc)
	if err != nil {
	...
	if err := testClient.CoreV1().PersistentVolumeClaims(ns.Name).Delete(ctx, pvc.Name, nil); err != nil {
...
func TestPersistentVolumeBindRace(t *testing.T) {
	...
	for counter <= maxClaims {
		...
		claim, err := testClient.CoreV1().PersistentVolumeClaims(ns.Name).Create(ctx, newPvc)
		if err != nil {
	...
	pv, err = testClient.CoreV1().PersistentVolumes().Create(ctx, pv)
	if err != nil {
	...
	pv, err = testClient.CoreV1().PersistentVolumes().Get(ctx, pv.Name, metav1.GetOptions{})
	if err != nil {
...
func TestPersistentVolumeClaimLabelSelector(t *testing.T) {
	...
	_, err = testClient.CoreV1().PersistentVolumes().Create(ctx, pv_true)
	if err != nil {
		t.Fatalf("Failed to create PersistentVolume: %v", err)
	}
	_, err = testClient.CoreV1().PersistentVolumes().Create(ctx, pv_false)
	if err != nil {
		t.Fatalf("Failed to create PersistentVolume: %v", err)
	}
	...
	pv, err := testClient.CoreV1().PersistentVolumes().Get(ctx, "pv-false", metav1.GetOptions{})
	if err != nil {
		t.Fatalf("Unexpected error getting pv: %v", err)
	}
	if pv.Spec.ClaimRef != nil {
	...
	pv, err = testClient.CoreV1().PersistentVolumes().Get(ctx, "pv-true", metav1.GetOptions{})
	if err != nil {
		t.Fatalf("Unexpected error getting pv: %v", err)
	}
	if pv.Spec.ClaimRef == nil {
...
func TestPersistentVolumeMultiPVs(t *testing.T) {
	...
	for i := 0; i < maxPVs; i++ {
		_, err := testClient.CoreV1().PersistentVolumes().Create(ctx, pvs[i])
		if err != nil {
	...
	bound := 0
	for i := 0; i < maxPVs; i++ {
		pv, err := testClient.CoreV1().PersistentVolumes().Get(ctx, pvs[i].Name, metav1.GetOptions{})
		if err != nil {
			t.Fatalf("Unexpected error getting pv: %v", err)
		}
		if pv.Spec.ClaimRef == nil {
...
func TestPersistentVolumeMultiPVsPVCs(t *testing.T) {
	...
	go func() {
		for i := 0; i < objCount; i++ {
			_, _ = testClient.CoreV1().PersistentVolumes().Create(ctx, pvs[i])
		}
	}()
	...
	go func() {
		for {
			...
			if rand.Intn(2) == 0 {
				...
				pv, err := testClient.CoreV1().PersistentVolumes().Get(ctx, name, metav1.GetOptions{})
				if err != nil {
				...
				_, err = testClient.CoreV1().PersistentVolumes().Update(ctx, pv)
				if err != nil {
				...
				pvc, err := testClient.CoreV1().PersistentVolumeClaims(metav1.NamespaceDefault).Get(ctx, name, metav1.GetOptions{})
				if err != nil {
				...
				_, err = testClient.CoreV1().PersistentVolumeClaims(metav1.NamespaceDefault).Update(ctx, pvc)
				if err != nil {
	...
	go func() {
		for i := 0; i < objCount; i++ {
			_, _ = testClient.CoreV1().PersistentVolumeClaims(ns.Name).Create(ctx, pvcs[i])
		}
	}()
	...
	for i := 0; i < objCount; i++ {
		...
		pvc, err := testClient.CoreV1().PersistentVolumeClaims(ns.Name).Get(ctx, pvcs[i].Name, metav1.GetOptions{})
		if err != nil {
...
func TestPersistentVolumeControllerStartup(t *testing.T) {
	...
	for i := 0; i < objCount; i++ {
		...
		newPVC, err := testClient.CoreV1().PersistentVolumeClaims(ns.Name).Create(ctx, pvc)
		if err != nil {
		...
		newPVC, err = testClient.CoreV1().PersistentVolumeClaims(ns.Name).UpdateStatus(ctx, newPVC)
		if err != nil {
		...
		pv.Spec.ClaimRef = claimRef
		newPV, err := testClient.CoreV1().PersistentVolumes().Create(ctx, pv)
		if err != nil {
			t.Fatalf("Cannot create volume %q: %v", pv.Name, err)
		}
		// Save Bound status as a separate transaction
		newPV.Status.Phase = v1.VolumeBound
		newPV, err = testClient.CoreV1().PersistentVolumes().UpdateStatus(ctx, newPV)
		if err != nil {
...
func TestPersistentVolumeProvisionMultiPVCs(t *testing.T) {
	...
	defer testClient.Core().PersistentVolumes().DeleteCollection(ctx, nil, metav1.ListOptions{})
	defer testClient.StorageV1().StorageClasses().DeleteCollection(ctx, nil, metav1.ListOptions{})
	...
	testClient.StorageV1().StorageClasses().Create(ctx, &storageClass)
	...
	pvList, err := testClient.CoreV1().PersistentVolumes().List(ctx, metav1.ListOptions{})
	if err != nil {
	...
	// Delete the claims
	for i := 0; i < objCount; i++ {
		_ = testClient.CoreV1().PersistentVolumeClaims(ns.Name).Delete(ctx, pvcs[i].Name, nil)
	}

	// Wait for the PVs to get deleted by listing remaining volumes
	// (delete events were unreliable)
	for {
		volumes, err := testClient.CoreV1().PersistentVolumes().List(ctx, metav1.ListOptions{})
		if err != nil {
...
func TestPersistentVolumeMultiPVsDiffAccessModes(t *testing.T) {
	...
	_, err := testClient.CoreV1().PersistentVolumes().Create(ctx, pv_rwm)
	if err != nil {
		t.Errorf("Failed to create PersistentVolume: %v", err)
	}
	_, err = testClient.CoreV1().PersistentVolumes().Create(ctx, pv_rwo)
	if err != nil {
		t.Errorf("Failed to create PersistentVolume: %v", err)
	}
	t.Log("volumes created")
	...
	pv, err := testClient.CoreV1().PersistentVolumes().Get(ctx, "pv-rwo", metav1.GetOptions{})
	if err != nil {
		t.Fatalf("Unexpected error getting pv: %v", err)
	}
	if pv.Spec.ClaimRef != nil {
	...
	pv, err = testClient.CoreV1().PersistentVolumes().Get(ctx, "pv-rwm", metav1.GetOptions{})
	if err != nil {
		t.Fatalf("Unexpected error getting pv: %v", err)
	}
	if pv.Spec.ClaimRef == nil {
...
func waitForPersistentVolumePhase(client *clientset.Clientset, pvName string, w watch.Interface, phase v1.PersistentVolumePhase) {
	...
	volume, err := client.Core().PersistentVolumes().Get(context.TODO(), pvName, metav1.GetOptions{})
...
func waitForPersistentVolumeClaimPhase(client *clientset.Clientset, claimName, namespace string, w watch.Interface, phase v1.PersistentVolumeClaimPhase) {
	...
	claim, err := client.Core().PersistentVolumeClaims(namespace).Get(context.TODO(), claimName, metav1.GetOptions{})
...
func createClients(ns *v1.Namespace, t *testing.T, s *httptest.Server, syncPeriod time.Duration) (*clientset.Clientset, *persistentvolumecontroller.PersistentVolumeController, informers.SharedInformerFactory, watch.Interface, watch.Interface) {
	...
	watchPV, err := testClient.CoreV1().PersistentVolumes().Watch(ctx, metav1.ListOptions{})
	if err != nil {
		t.Fatalf("Failed to watch PersistentVolumes: %v", err)
	}
	watchPVC, err := testClient.CoreV1().PersistentVolumeClaims(ns.Name).Watch(ctx, metav1.ListOptions{})
	if err != nil {
