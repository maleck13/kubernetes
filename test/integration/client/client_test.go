...
func TestClient(t *testing.T) {
	...
	pods, err := client.Core().Pods(ns.Name).List(ctx, metav1.ListOptions{})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(pods.Items) != 0 {
	...
	got, err := client.Core().Pods(ns.Name).Create(ctx, pod)
	if err == nil {
...
func TestAtomicPut(t *testing.T) {
	...
	for label, value := range testLabels {
		go func(l, v string) {
			defer wg.Done()
			for {
				tmpRC, err := rcs.Get(ctx, rc.Name, metav1.GetOptions{})
				if err != nil {
				...
				tmpRC, err = rcs.Update(ctx, tmpRC)
				if err != nil {
	...
	wg.Wait()
	rc, err = rcs.Get(ctx, rc.Name, metav1.GetOptions{})
	if err != nil {
...
func TestPatch(t *testing.T) {
	...
	pod, err := pods.Create(ctx, &podBody)
	if err != nil {
...
func TestPatchWithCreateOnUpdate(t *testing.T) {
	...
	createdEndpoint, err := c.Core().Endpoints(ns.Name).Update(ctx, endpointTemplate)
	if err != nil {
...
func TestSingleWatch(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		event := mkEvent(i)
		got, err := client.Core().Events(ns.Name).Create(ctx, event)
		if err != nil {
...
func TestMultiWatch(t *testing.T) {
	...
	for i := 0; i < watcherCount; i++ {
		...
		got, err := client.Core().Pods(ns.Name).Create(ctx, &v1.Pod{
			ObjectMeta: metav1.ObjectMeta{
				Name:   name,
		...
		go func(name, rv string) {
			...
			w, err := client.Core().Pods(ns.Name).Watch(ctx, options)
			if err != nil {
		...
		for i := 0; i < 50; i++ {
			...
			go func() {
				...
				for {
					...
					if _, err := client.Core().Events(ns.Name).Create(ctx, dummyEvent(i)); err != nil {
		...
		for i := 0; i < 50; i++ {
			...
			go func() {
				...
				for {
					...
					name := fmt.Sprintf("unrelated-%v", i)
					_, err := client.Core().Pods(ns.Name).Create(ctx, &v1.Pod{
						ObjectMeta: metav1.ObjectMeta{
							Name: name,
						},
						Spec: v1.PodSpec{
	...
	for i := 0; i < watcherCount; i++ {
		go func(i int) {
			name := fmt.Sprintf("multi-watch-%v", i)
			pod, err := client.Core().Pods(ns.Name).Get(ctx, name, metav1.GetOptions{})
			if err != nil {
			...
			if _, err := client.Core().Pods(ns.Name).Update(ctx, pod); err != nil {
...
func runSelfLinkTestOnNamespace(t *testing.T, c clientset.Interface, namespace string) {
	...
	pod, err := c.CoreV1().Pods(namespace).Create(ctx, &podBody)
	if err != nil {
