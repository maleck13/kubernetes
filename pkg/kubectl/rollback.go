...
func (r *DeploymentRollbacker) Rollback(obj runtime.Object, updatedAnnotations map[string]string, toRevision int64, dryRun bool) (string, error) {
	...
	ctx := context.TODO()
	// Get current events
	events, err := r.c.CoreV1().Events(d.Namespace).List(ctx, metav1.ListOptions{})
	if err != nil {
		return result, err
	}
	// Do the rollback
	...
	watch, err := r.c.CoreV1().Events(d.Namespace).Watch(ctx, metav1.ListOptions{Watch: true, ResourceVersion: events.ResourceVersion})
	if err != nil {
		return result, err
	}
