...
func (dsc *DaemonSetsController) constructHistory(ds *apps.DaemonSet) (cur *apps.ControllerRevision, old []*apps.ControllerRevision, err error) {
	...
	for _, history := range histories {
		...
		if _, ok := history.Labels[apps.DefaultDaemonSetUniqueLabelKey]; !ok {
			...
			history, err = dsc.kubeClient.AppsV1().ControllerRevisions(ds.Namespace).Update(ctx, toUpdate)
			if err != nil {
				return nil, nil, err
			}
		}
	...
	default:
		...
		if cur.Revision < currRevision {
			...
			_, err = dsc.kubeClient.AppsV1().ControllerRevisions(ds.Namespace).Update(ctx, toUpdate)
			if err != nil {
				return nil, nil, err
			}
		}
	...
	for _, cur := range curHistories {
		...
		for _, pod := range pods {
			if pod.Labels[apps.DefaultDaemonSetUniqueLabelKey] != keepCur.Labels[apps.DefaultDaemonSetUniqueLabelKey] {
				...
				_, err = dsc.kubeClient.CoreV1().Pods(ds.Namespace).Update(ctx, toUpdate)
				if err != nil {
					return nil, err
				}
			}
		...
		// Remove duplicates
		err = dsc.kubeClient.AppsV1beta1().ControllerRevisions(ds.Namespace).Delete(ctx, cur.Name, nil)
		if err != nil {
			return nil, err
		}
	}
...
func (dsc *DaemonSetsController) snapshot(ds *apps.DaemonSet, revision int64) (*apps.ControllerRevision, error) {
	...
	history, err = dsc.kubeClient.AppsV1().ControllerRevisions(ds.Namespace).Create(ctx, history)
	if errors.IsAlreadyExists(err) {
		...
		currDS, getErr := dsc.kubeClient.ExtensionsV1beta1().DaemonSets(ds.Namespace).Get(ctx, ds.Name, metav1.GetOptions{})
		if getErr != nil {
			return nil, getErr
		}
		...
		*currDS.Status.CollisionCount++
		_, updateErr := dsc.kubeClient.ExtensionsV1beta1().DaemonSets(ds.Namespace).UpdateStatus(ctx, currDS)
		if updateErr != nil {
