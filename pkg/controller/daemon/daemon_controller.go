...
func NewDaemonSetsController(daemonSetInformer appsinformers.DaemonSetInformer, historyInformer appsinformers.ControllerRevisionInformer, podInformer coreinformers.PodInformer, nodeInformer coreinformers.NodeInformer, kubeClient clientset.Interface) (*DaemonSetsController, error) {
	...
	daemonSetInformer.Informer(ctx).AddEventHandler(cache.ResourceEventHandlerFuncs{
	...
	dsc.dsLister = daemonSetInformer.Lister(ctx)
	dsc.dsStoreSynced = daemonSetInformer.Informer(ctx).HasSynced
	...
	dsc.historyLister = historyInformer.Lister(ctx)
	dsc.historyStoreSynced = historyInformer.Informer(ctx).HasSynced
	...
	dsc.podLister = podInformer.Lister(ctx)
	...
	podInformer.Informer(ctx).GetIndexer().AddIndexers(cache.Indexers{
		"nodeName": indexByPodNodeName,
	})
	dsc.podNodeIndex = podInformer.Informer(ctx).GetIndexer()
	dsc.podStoreSynced = podInformer.Informer(ctx).HasSynced

	nodeInformer.Informer(ctx).AddEventHandler(cache.ResourceEventHandlerFuncs{
		AddFunc:    dsc.addNode,
		UpdateFunc: dsc.updateNode,
	},
	)
	dsc.nodeStoreSynced = nodeInformer.Informer(ctx).HasSynced
	dsc.nodeLister = nodeInformer.Lister(ctx)
