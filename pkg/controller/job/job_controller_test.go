...
func TestControllerSyncJob(t *testing.T) {
	...
	for name, tc := range testCases {
		...
		sharedInformerFactory.Batch().V1().Jobs().Informer(ctx).GetIndexer().Add(job)
		podIndexer := sharedInformerFactory.Core().V1().Pods().Informer(ctx).GetIndexer()
...
func TestGetPodsForJob(t *testing.T) {
	...
	job2 := newJob(1, 1, 6)
	job2.Name = "job2"
	informer.Batch().V1().Jobs().Informer(ctx).GetIndexer().Add(job1)
	informer.Batch().V1().Jobs().Informer(ctx).GetIndexer().Add(job2)
	...
	pod3.Labels = nil
	informer.Core().V1().Pods().Informer(ctx).GetIndexer().Add(pod1)
	informer.Core().V1().Pods().Informer(ctx).GetIndexer().Add(pod2)
	informer.Core().V1().Pods().Informer(ctx).GetIndexer().Add(pod3)
...
func TestGetPodsForJobNoAdoptIfBeingDeletedRace(t *testing.T) {
	...
	informer.Batch().V1().Jobs().Informer(ctx).GetIndexer().Add(&cachedJob)
...
func TestAddPodOrphan(t *testing.T) {
	...
	informer.Batch().V1().Jobs().Informer(ctx).GetIndexer().Add(job1)
	informer.Batch().V1().Jobs().Informer(ctx).GetIndexer().Add(job2)
	informer.Batch().V1().Jobs().Informer(ctx).GetIndexer().Add(job3)
...
func TestWatchPods(t *testing.T) {
	...
	sharedInformerFactory.Batch().V1().Jobs().Informer(ctx).GetIndexer().Add(testJob)
	received := make(chan struct{})
	...
	stopCh := make(chan struct{})
	defer close(stopCh)
	go sharedInformerFactory.Core().V1().Pods().Informer(ctx).Run(stopCh)
...
func TestJobBackoffReset(t *testing.T) {
	...
	for name, tc := range testCases {
		...
		job = actual
		sharedInformerFactory.Batch().V1().Jobs().Informer(ctx).GetIndexer().Replace([]interface{}{actual}, actual.ResourceVersion)
