...
func NewDisruptionController(
	...
	pdbInformer.Informer(ctx).AddEventHandlerWithResyncPeriod(
	...
	dc.pdbLister = pdbInformer.Lister(ctx)
	dc.pdbListerSynced = pdbInformer.Informer(ctx).HasSynced

	dc.rcLister = rcInformer.Lister(ctx)
	dc.rcListerSynced = rcInformer.Informer(ctx).HasSynced
	...
	dc.ssLister = ssInformer.Lister(ctx)
	dc.ssListerSynced = ssInformer.Informer(ctx).HasSynced
