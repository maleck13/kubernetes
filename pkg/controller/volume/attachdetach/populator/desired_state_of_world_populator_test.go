...
func TestFindAndAddActivePods_FindAndRemoveDeletedPods(t *testing.T) {
	...
	pvcLister := fakeInformerFactory.Core().V1().PersistentVolumeClaims().Lister(ctx)
	pvLister := fakeInformerFactory.Core().V1().PersistentVolumes().Lister(ctx)
	...
	dswp := &desiredStateOfWorldPopulator{
		...
		podLister:             fakePodInformer.Lister(ctx),
		pvcLister:             pvcLister,
		pvLister:              pvLister,
	}
	...
	fakePodInformer.Informer(ctx).GetStore().Delete(pod)
