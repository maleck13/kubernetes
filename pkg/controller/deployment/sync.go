...
func (dc *DeploymentController) checkPausedConditions(d *extensions.Deployment) error {
	...
	var err error
	d, err = dc.client.ExtensionsV1beta1().Deployments(d.Namespace).UpdateStatus(context.TODO(), d)
	return err
}
...
func (dc *DeploymentController) getNewReplicaSet(d *extensions.Deployment, rsList, oldRSs []*extensions.ReplicaSet, createIfNotExisted bool) (*extensions.ReplicaSet, error) {
	...
	if existingNewRS != nil {
		...
		if annotationsUpdated || minReadySecondsNeedsUpdate {
			...
			return dc.client.ExtensionsV1beta1().ReplicaSets(rsCopy.ObjectMeta.Namespace).Update(ctx, rsCopy)
		}
		...
		if needsUpdate {
			var err error
			if d, err = dc.client.ExtensionsV1beta1().Deployments(d.Namespace).UpdateStatus(ctx, d); err != nil {
				return nil, err
			}
		}
		return rsCopy, nil
	...
	*(newRS.Spec.Replicas) = newReplicasCount
	...
	// the Deployment.
	alreadyExists := false
	createdRS, err := dc.client.ExtensionsV1beta1().ReplicaSets(d.Namespace).Create(ctx, &newRS)
	switch {
	...
	case errors.IsAlreadyExists(err):
		...
		*d.Status.CollisionCount++
		...
		// error.
		_, dErr := dc.client.ExtensionsV1beta1().Deployments(d.Namespace).UpdateStatus(ctx, d)
		if dErr == nil {
	...
	case err != nil:
		...
		if d.Spec.ProgressDeadlineSeconds != nil {
			...
			_, _ = dc.client.ExtensionsV1beta1().Deployments(d.Namespace).UpdateStatus(ctx, d)
		}
	...
	if needsUpdate {
		_, err = dc.client.ExtensionsV1beta1().Deployments(d.Namespace).UpdateStatus(ctx, d)
	}
...
func (dc *DeploymentController) scaleReplicaSet(rs *extensions.ReplicaSet, newScale int32, deployment *extensions.Deployment, scalingOperation string) (bool, *extensions.ReplicaSet, error) {
	...
	if sizeNeedsUpdate || annotationsNeedUpdate {
		...
		*(rsCopy.Spec.Replicas) = newScale
		...
		rs, err = dc.client.ExtensionsV1beta1().ReplicaSets(rsCopy.Namespace).Update(context.TODO(), rsCopy)
...
func (dc *DeploymentController) cleanupDeployment(oldRSs []*extensions.ReplicaSet, deployment *extensions.Deployment) error {
	...
	for i := int32(0); i < diff; i++ {
		...
		if err := dc.client.ExtensionsV1beta1().ReplicaSets(rs.Namespace).Delete(context.TODO(), rs.Name, nil); err != nil && !errors.IsNotFound(err) {
...
func (dc *DeploymentController) syncDeploymentStatus(allRSs []*extensions.ReplicaSet, newRS *extensions.ReplicaSet, d *extensions.Deployment) error {
	...
	_, err := dc.client.ExtensionsV1beta1().Deployments(newDeployment.Namespace).UpdateStatus(context.TODO(), newDeployment)
	return err
}
