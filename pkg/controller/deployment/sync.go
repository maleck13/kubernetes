	...
	*(newRS.Spec.Replicas) = newReplicasCount
	...
	case errors.IsAlreadyExists(err):
		...
		*d.Status.CollisionCount++
		...
		// error.
		_, dErr := dc.client.AppsV1().Deployments(d.Namespace).UpdateStatus(ctx, d)
		if dErr == nil {
	...
	case err != nil:
		...
		if d.Spec.ProgressDeadlineSeconds != nil {
			...
			_, _ = dc.client.AppsV1().Deployments(d.Namespace).UpdateStatus(ctx, d)
		}
	...
	if needsUpdate {
		_, err = dc.client.AppsV1().Deployments(d.Namespace).UpdateStatus(ctx, d)
	}
