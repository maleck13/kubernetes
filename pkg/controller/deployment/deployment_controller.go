...
func NewDeploymentController(dInformer extensionsinformers.DeploymentInformer, rsInformer extensionsinformers.ReplicaSetInformer, podInformer coreinformers.PodInformer, client clientset.Interface) (*DeploymentController, error) {
	...
	dInformer.Informer(ctx).AddEventHandler(cache.ResourceEventHandlerFuncs{
		AddFunc:    dc.addDeployment,
		UpdateFunc: dc.updateDeployment,
		// This will enter the sync loop and no-op, because the deployment has been deleted from the store.
		DeleteFunc: dc.deleteDeployment,
	})
	rsInformer.Informer(ctx).AddEventHandler(cache.ResourceEventHandlerFuncs{
	...
	dc.dLister = dInformer.Lister(ctx)
	dc.rsLister = rsInformer.Lister(ctx)
	dc.podLister = podInformer.Lister(ctx)
	dc.dListerSynced = dInformer.Informer(ctx).HasSynced
	dc.rsListerSynced = rsInformer.Informer(ctx).HasSynced
	dc.podListerSynced = podInformer.Informer(ctx).HasSynced
	return dc, nil
}
...
func (dc *DeploymentController) getReplicaSetsForDeployment(d *extensions.Deployment) ([]*extensions.ReplicaSet, error) {
	...
	canAdoptFunc := controller.RecheckDeletionTimestamp(func() (metav1.Object, error) {
		fresh, err := dc.client.ExtensionsV1beta1().Deployments(d.Namespace).Get(context.TODO(), d.Name, metav1.GetOptions{})
		if err != nil {
			return nil, err
		}
		if fresh.UID != d.UID {
...
func (dc *DeploymentController) syncDeployment(key string) error {
	...
	if reflect.DeepEqual(d.Spec.Selector, &everything) {
		...
		if d.Status.ObservedGeneration < d.Generation {
			...
			dc.client.ExtensionsV1beta1().Deployments(d.Namespace).UpdateStatus(context.TODO(), d)
		}
		return nil
	}
