...
func DeletePods(kubeClient clientset.Interface, recorder record.EventRecorder, nodeName, nodeUID string, daemonStore extensionslisters.DaemonSetLister) (bool, error) {
	...
	pods, err := kubeClient.CoreV1().Pods(metav1.NamespaceAll).List(ctx, options)
	...
	for _, pod := range pods.Items {
		...
		if err := kubeClient.CoreV1().Pods(pod.Namespace).Delete(ctx, pod.Name, nil); err != nil {
			return false, err
		}
		remaining = true
	}
...
func ForcefullyDeleteNode(kubeClient clientset.Interface, nodeName string) error {
	if err := kubeClient.CoreV1().Nodes().Delete(context.TODO(), nodeName, nil); err != nil {
...
func MarkAllPodsNotReady(kubeClient clientset.Interface, node *v1.Node) error {
	...
	pods, err := kubeClient.CoreV1().Pods(metav1.NamespaceAll).List(ctx, opts)
	if err != nil {
		return err
	}
	...
	for _, pod := range pods.Items {
		...
		for i, cond := range pod.Status.Conditions {
			if cond.Type == v1.PodReady {
				...
				_, err := kubeClient.CoreV1().Pods(pod.Namespace).UpdateStatus(ctx, &pod)
				if err != nil {
