...
func NewTokensController(serviceAccounts informers.ServiceAccountInformer, secrets informers.SecretInformer, cl clientset.Interface, options TokensControllerOptions) (*TokensController, error) {
	...
	ctx := context.TODO()
	e.serviceAccounts = serviceAccounts.Lister(ctx)
	e.serviceAccountSynced = serviceAccounts.Informer(ctx).HasSynced
	serviceAccounts.Informer(ctx).AddEventHandlerWithResyncPeriod(
...
func (e *TokensController) ensureReferencedToken(serviceAccount *v1.ServiceAccount) ( /* retry */ bool, error) {
	...
	// Save the secret
	createdToken, err := e.client.CoreV1().Secrets(serviceAccount.Namespace).Create(ctx, secret)
	if err != nil {
		// retriable error
		return true, err
	}
	...
	err = clientretry.RetryOnConflict(clientretry.DefaultRetry, func() error {
		...
		if _, err := serviceAccounts.Update(ctx, liveServiceAccount); err != nil {
			return err
		}
	...
	if !addedReference {
		...
		if deleteErr := e.client.CoreV1().Secrets(createdToken.Namespace).Delete(ctx, createdToken.Name, deleteOpts); deleteErr != nil {
...
func (e *TokensController) generateTokenIfNeeded(serviceAccount *v1.ServiceAccount, cachedSecret *v1.Secret) ( /* retry */ bool, error) {
	...
	// Save the secret
	_, err = secrets.Update(ctx, liveSecret)
	if apierrors.IsConflict(err) || apierrors.IsNotFound(err) {
...
func (e *TokensController) removeSecretReference(saNamespace string, saName string, saUID types.UID, secretName string) error {
	...
	_, err = serviceAccounts.Update(ctx, serviceAccount)
	// Ignore NotFound errors when attempting to remove a reference
	if apierrors.IsNotFound(err) {
		return nil
	}
