...
func NewServiceAccountsController(saInformer coreinformers.ServiceAccountInformer, nsInformer coreinformers.NamespaceInformer, cl clientset.Interface, options ServiceAccountsControllerOptions) (*ServiceAccountsController, error) {
	...
	ctx := context.TODO()
	saInformer.Informer(ctx).AddEventHandler(cache.ResourceEventHandlerFuncs{
		DeleteFunc: e.serviceAccountDeleted,
	})
	e.saLister = saInformer.Lister(ctx)
	e.saListerSynced = saInformer.Informer(ctx).HasSynced

	nsInformer.Informer(ctx).AddEventHandler(cache.ResourceEventHandlerFuncs{
		AddFunc:    e.namespaceAdded,
		UpdateFunc: e.namespaceUpdated,
	})
	e.nsLister = nsInformer.Lister(ctx)
	e.nsListerSynced = nsInformer.Informer(ctx).HasSynced
